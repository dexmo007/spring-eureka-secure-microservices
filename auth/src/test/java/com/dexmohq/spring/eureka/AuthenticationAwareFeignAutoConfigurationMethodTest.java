 ...
 * @author Henrik Drefs
 */
public class AuthenticationAwareFeignAutoConfigurationMethodTest extends AuthenticationAwareFeignAutoConfigurationBase {

    @FeignClient(name = "method-client", url = "http://foo.bar")
    interface MethodClient {
    ...
    @Configuration
    @EnableFeignClients(clients = MethodClient.class)
    @CommonsLog
    static class Config extends BaseConfig {
        @Bean
        CommandLineRunner runner(MethodClient client) {
            return args -> {
    ...
    public void testFeignClient_WithAuthAnnotationsOnMethod_AuthenticateAsUser() {
        testFeignClient(Config.class, MethodClient.class, client -> {
    ...
    public void testFeignClient_WithAuthAnnotationsOnMethod_AuthenticateAsSystem() {
        testFeignClient(Config.class, MethodClient.class, MethodClient::testAsSystem, request -> {
            assertThat(request.headers()).hasEntrySatisfying("X-Auth-Type", values -> {
    ...
    public void testFeignClient_WithAuthAnnotationsOnMethod_ExplicitNoAuthentication() {
        testFeignClient(Config.class, MethodClient.class, MethodClient::testNoAuthentication, request -> {
    ...
    @Test
    public void testFeignClient_WithAuthAnnotationsOnMethod_ImplicitNoAuthentication() {
        testFeignClient(Config.class, MethodClient.class, MethodClient::test, request -> {
            assertThat(request.headers()).doesNotContainKey("X-Auth-Type");
            assertThat(request.headers()).doesNotContainKey("Authorization");
        });
    }
